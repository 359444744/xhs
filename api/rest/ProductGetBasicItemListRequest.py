...
class ProductGetBasicItemListRequest(RestApi):
    """
    ...
    @:param: skucode:string:小红书编码
    @:param: freeze:boolean:是否被冻结 true为已冻结，false为未冻结
    @:param: singlePackOnly:boolean:只返回单品类型的商品
    @:param: lastId:string:查询起始商品id，会返回这个商品后pagesize个商品，全店商品时间倒序，全店商品同步请使用lastid循环拉取
    ...
    def getapiname(self):
        return 'product.getBasicItemList'